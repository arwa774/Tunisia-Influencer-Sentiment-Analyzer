...
# tiktok_users=df["username"]
# list_username=tiktok_users
# # --- 1. CHECKPOINT SYSTEM SETUP ---
# # Load existing progress if the checkpoint file exists
...
#     # Skip if the user has already been completely scraped
#     if username in completed_users:
#         print(f"[SKIP] User @{username} already fully processed.")
#         continue
#
...
#
#             # Check if this specific video URL is already in our checkpoint
#             if video_url in completed_videos:
#                 print(f"[SKIP] [{i}/{len(videos_urls)}] Video already scraped: {video_url}")
#                 if i == 10:  # Keep your original break condition
#                     break
#                 continue
#
#             print(f"[SCRAPING][{i}/{len(videos_urls)}]: {video_url}")
#
#             comments = asyncio.run(
#                 scrape_tiktok_comments(video_url=video_url, max_comments=300, proxy=YOUR_PROXY)
#             )
#
...
#                 })
#
#             # Flush to physically write the data to the CSV on the hard drive right now
#             f.flush()
#
#             # Add this video to our completed list and save the checkpoint
...
#                 break
#
#     # Once loop is done (or hits 10), mark the whole user as complete
